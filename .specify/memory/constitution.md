...
### I. 規格驅動開發 (Specification-Driven Development)
所有開發工作，包括新功能和錯誤修復，都必須嚴格遵循本文件下方「技術與功能規格」章節所定義的規格。該章節是技術和功能需求的唯一真實來源。

### II. 嚴格的類型安全 (Strict Type Safety)
本專案使用 TypeScript 建構，且絕不妥協地啟用 `strict` 模式。所有程式碼都必須是強型別，以確保穩健性、可維護性，並在編譯時期捕捉錯誤。除非有明確且記錄在案的理由，否則禁止使用 `any` 類型。

### III. 設計即安全與隱私 (Secure and Private by Design)
本擴充功能必須遵守「最小權限原則」，僅請求其運作所必需的權限 (`storage`, `tabs`)。所有使用者輸入都必須經過驗證。使用者隱私至關重要，所有資料都應使用 `chrome.storage.local` 在本機儲存，絕不傳輸到外部伺服器。

### IV. 明確的關注點分離 (Clear Separation of Concerns)
背景服務工作執行緒 (`src/background.ts`) 和使用者介面邏輯 (`src/popup.ts`) 之間必須保持清晰的架構分離。這有助於提升模組化、簡化除錯，並使程式碼庫更易於理解和擴展。

### V. 透過測試確保品質 (Quality Through Testing)
雖然測試尚未完全實施，但對品質的承諾至關重要。所有新功能在開發時都應考慮其可測試性，未來的開發工作必須包含建立一個全面的測試套件，如本文件「測試規格」部分所述。

## 技術堆疊

本專案將遵循以下技術堆疊：
- **語言**: TypeScript (並啟用 `strict` 模式)
- **平台**: Chrome 擴充功能 (Manifest V3)
- **資料儲存**: `chrome.storage.local` API

## 開發工作流程

開發流程如下：
1.  **建置**: 執行 `npm run build` 命令，將 `src/` 目錄下的 TypeScript 原始碼編譯到 `dist/` 目錄。
2.  **監控**: 在開發過程中，可執行 `npm run watch` 來自動監控檔案變更並重新編譯。
3.  **執行**: 在 Chrome 的開發人員模式中，選擇專案的 `dist/` 目錄作為「未封裝的擴充功能」來載入。
4.  **除錯**: 使用 Chrome 的開發人員工具對彈出視窗和背景服務工作執行緒進行除錯。

## 治理

本憲章是指導「自訂書籤管理器」擴充功能開發的基礎文件。所有開發活動和程式碼審查都必須確保遵循這些原則。對本憲章的任何修訂都需要經過討論、達成共識，並更新此文件。

**版本**: 1.0.0 | **批准日期**: 2025-10-18 | **最後修訂**: 2025-10-18
...
```
請參考 `.specify/memory/constitution.md` 檔案，
該文件包含專案的核心原則、完整的技術架構、功能規格、程式碼規範等資訊。
```

或針對特定部分:

```
請根據 `constitution.md` 中的「資料結構規格」進行開發
請遵循 `constitution.md` 中的「程式碼品質規範」
請參考 `constitution.md` 中的「擴充性規格」來實作新功能
```