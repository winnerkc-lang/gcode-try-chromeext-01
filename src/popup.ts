...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const bookmarksList = document.getElementById('bookmarks-list') as HTMLDivElement;
  ...
  function loadBookmarks() {
    chrome.runtime.sendMessage({ action: 'get-bookmarks' }, (bookmarks: Bookmark[]) => {
  ...
  // 處理點擊事件 (開啟分頁或刪除)
  bookmarksList.addEventListener('click', (e: MouseEvent) => {
    const target = e.target as HTMLElement;
    const item = target.closest('.bookmark-item') as HTMLDivElement;
    if (!item) return;
